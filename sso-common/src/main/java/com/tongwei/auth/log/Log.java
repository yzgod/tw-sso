...
public class Log {
    ...
    public void logToDbAccess(String msg, String method, String url, String ip, String parameter, Integer timeUsed) {
        String loginName = SessionCore.LoginName.value();
        if (loginName != null) {
            DBAccessUserLogBean dbLogBean = new DBAccessUserLogBean(appCode, msg, loginName, method, url, ip, parameter,
                    timeUsed);
            dbLogBean.setType(2);
            addToLogQueue(dbLogBean);
        } else {
    ...
    private void logUserToFile(Class<?> clz, String msg, Throwable error) {
        String loginName = SessionCore.LoginName.value();
        if (loginName != null) {
            TextUserLogBean logBean = new TextUserLogBean(appCode, msg, error, clz, Thread.currentThread().getName(),
                    loginName);
            logBean.setType(12);
            addToLogQueue(logBean);
        } else {
