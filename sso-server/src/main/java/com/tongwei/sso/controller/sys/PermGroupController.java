...
public class PermGroupController extends BaseController {
    ...
    public Object getPermGroupTree() {
        ...
        for (RegisterApp app : apps) {
            ...
            List<PermGroup> src = perms.stream().filter(e -> e.getAppCode().equals(app.getAppCode()))
    ...
    public Object getPgByParentId(String appCode, String pId) {
        List<PermGroup> list = permGroupService.findByPropAsc(new String[] { "appCode", "parentId" },
    ...
    public Result save(PermGroup pg) {
        if (!AppUtils.validateCode(pg.getAppCode())) {
            return ResultUtil.doFailure("非法的应用编码!");
        }
        if (!AppUtils.validateCode(pg.getCode())) {
        ...
        if (pg.getId() == null) {
            boolean b = permGroupService.checkIfExist(new String[] { "appCode", "code" },
                    new String[] { pg.getAppCode(), pg.getCode() });
            if (b) {
    ...
    private void recPgTree(PermGroup pg, List<PermGroup> src) {
        List<PermGroup> children = src.stream().filter(e -> e.getParentId().equals(pg.getId()))
                .collect(Collectors.toList());
        for (PermGroup child : children) {
            List<PermGroup> cd = src.stream().filter(e -> e.getParentId().equals(child.getId()))
                    .collect(Collectors.toList());
            recPgTree(child, src);
