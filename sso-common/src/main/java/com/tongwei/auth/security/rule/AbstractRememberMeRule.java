...
public abstract class AbstractRememberMeRule implements RememberMeRule {
	...
	@Override
	public boolean validate(HttpServletRequest request, HttpServletResponse response, FilterChain chain,RememberMeType type,int rememberMeExpireTime) {
	...
	private boolean handleExpireTime(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
			int rememberMeExpireTime, Integer userId, long start) throws IOException, ServletException, InterruptedException {
		long cur = System.currentTimeMillis();
