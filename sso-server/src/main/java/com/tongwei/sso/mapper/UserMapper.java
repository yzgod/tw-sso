...
public interface UserMapper extends CmMapper<User> {
	...
	void saveUserOrg(@Param("userId") Integer userId,@Param("orgId") Integer orgId,@Param("isDefault")boolean isDefault);
	
	void deleteUserOrg(Integer userId);
	
	void saveUserPosition(@Param("userId") Integer userId,@Param("positionId") Integer positionId,@Param("isDefault")boolean isDefault);
